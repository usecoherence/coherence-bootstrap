...
Operators select the logical catalog with **`DOLT_DB`**, or derive it from the manifest when unset (normalized slug + hash + env tier after **`project init`**, or legacy **`dolt_db_name`** — see the env table). **`ConnectionConfig::from_env()`** applies matching defaults when **`COHERENCE_USE_USER_SCOPED_DOLT`** is set (socket path above + internal TCP port **33306** for Refinery unless **`DOLT_PORT`** / **`COHERENCE_DOLT_TCP_PORT`** override). CLI connections still prefer **`DOLT_SOCKET`**; TCP remains an implementation detail for embedded migrations.
