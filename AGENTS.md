...
With **`COHERENCE_USE_USER_SCOPED_DOLT`** enabled and **without** explicit `DOLT_DB`, **`migrate`** refuses to run if `.coherence/project.toml` exists but neither **`project_hash`** nor legacy **`dolt_db_name`** is set (run **`project init`** or set `DOLT_DB` manually) so migrations never target an accidental default catalog name.
...
Operators select the logical catalog with **`DOLT_DB`**, or derive it from the manifest when unset (normalized slug + hash + env tier after **`project init`**, or legacy **`dolt_db_name`** — see the env table). **`ConnectionConfig::from_env()`** applies matching defaults when **`COHERENCE_USE_USER_SCOPED_DOLT`** is set (socket path above + internal TCP port **33306** for **socket-fallback** connects unless **`DOLT_PORT`** / **`COHERENCE_DOLT_TCP_PORT`** override). CLI and embedded Refinery migrations use **`DOLT_SOCKET`** first (same **`mysql::Conn`** path); TCP is only a fallback when the unix socket is unavailable.
